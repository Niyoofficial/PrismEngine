...
{
	SetTextureRenderCommand(TextureView* inTextureView, const std::wstring& inParamName, int32_t inIndex)
		: textureView(inTextureView), paramName(inParamName), index(inIndex)
	{
	}

	void Execute(RenderCommandList* cmdList)
	{
		cmdList->SetTexture(textureView, paramName, index);
	}
...
DEFINE_RENDER_COMMAND(CopyTextureRegionFromTextureRenderCommand)
{
	...
	void Execute(RenderCommandList* cmdList)
	{
		cmdList->CopyTextureRegion(dest, destLoc, destSubresourceIndex, src, srcSubresourceIndex, srcLoc, srcSize);
	}
